...
source_dir = os.path.join(network_path, r"media\DMI\APR_data")
target_dir = os.path.join(network_path, r"media\DMI Clean_a\dialer_data")
...
try:
    ...
    if "Login Duration" in data.columns and "Total Breaks" in data.columns:
        ...
        print(f"'Net Login' column added successfully, and changes were saved to the original file: {target_path}")
        ...
        # Remove ICAI rows
        data = data[
            ~data.iloc[:, 0].astype(str).str.contains("ICAI", case=False, na=False)
        ]

        # Convert Agent Id to NUMBER
        data["Agent Id"] = (
            pd.to_numeric(data["Agent Id"], errors="coerce")
            .fillna(0)
            .astype(int)
        )
        ...
        # Convert First Login only if column exists
        if "First Login" in data.columns:
            data["First Login"] = pd.to_datetime(
                data["First Login"], errors="coerce"
            ).dt.strftime("%d-%b-%y")
...
try:
    ...
    final_df["Agent Id"] = (
        pd.to_numeric(final_df["Agent Id"], errors="coerce")
        .fillna(0)
        .astype(int)
    )

    xlsx_path = target_path.replace(".csv", ".xlsx")
...
# remote_dir = "/home/iccsadmin/APR_Data/DMI/APR_Clean"
remote_dir = "/home/iccsadmin/ishita/APR_Uploads_V2/media/dialer_watch/DMI"
