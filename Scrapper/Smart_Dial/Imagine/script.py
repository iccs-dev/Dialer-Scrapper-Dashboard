"""Imagine User Session scraper (Playwright).

Browser automation is Playwright driving the SmartDialPage page object; every
step runs through RecoveryRunner so a transient failure is retried and an
unrecoverable one lands in the central error handler with a screenshot, a
trace and an email alert. The pandas processing below the scrape is unchanged
from the Selenium version - same columns, same output files.
"""
...
# ==================== CONFIGURE PATHS ====================
# The process name comes from the folder this script lives in, and every
# folder below is built from that process plus the report date.
...
def scrape():
    ...
    try:
        ...
        destination, suggested = page_object.download_report(raw_download)
